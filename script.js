...
// ====== Local Storage (Auto Save Current Work) ======
...
function archiveCurrentReport() {
    ...
    const newReport = {
        ...
        title: title,
        weekNumber: parseInt(week) || 0, // Used for sorting chart
        dateSaved: dateSaved,
    ...
    // Replace if same week? For simplicity let's just push or update based on weekNumber
    const existingIndex = historyLog.findIndex(r => r.weekNumber === newReport.weekNumber);
    if (existingIndex >= 0) {
        if (confirm('يُوجد تقرير مسبق لنفس الأسبوع المؤرشف، هل تود تحديثه؟')) {
            historyLog[existingIndex] = newReport;
        } else {
            return;
        }
    } else {
        historyLog.push(newReport);
    }

    // Sort by week number
    historyLog.sort((a, b) => a.weekNumber - b.weekNumber);

    localStorage.setItem('reportsHistory', JSON.stringify(historyLog));
    showToast('تمت أرشفة التقرير بالنجاح!', 'success');
...
window.deleteHistoryReport = function (id) {
    if (confirm('هل أنت متأكد من حذف هذا التقرير المؤرشف؟')) {
        historyLog = historyLog.filter(r => r.id !== id);
        localStorage.setItem('reportsHistory', JSON.stringify(historyLog));
        renderHistoryChart();
        renderHistoryList();
    }
